...
function buildOverpassQuery(boundary, layers, options = {}) {
  const { maxFeatures = 5000, useBboxOnly = false, useCenterOnly = false } = options;

  // Use bounding box query instead of polygon for better compatibility
  if (useBboxOnly) {
    return buildBboxQuery(boundary, layers, maxFeatures, useCenterOnly);
  }
 ...
 */
function buildBboxQuery(boundary, layers, maxFeatures, useCenterOnly = false) {
