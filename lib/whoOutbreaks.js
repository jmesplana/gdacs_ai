...
const MAX_GEOCODE_RESULTS_PER_RESPONSE = 30;
...
function dedupeLocations(locations = []) {
  ...
  const countriesWithSpecificLocations = new Set(
    deduped
      .filter((location) => location.type !== 'country' && location.country)
      .map((location) => String(location.country).toLowerCase())
  );

  return deduped.filter((location) => {
    if (location.type !== 'country') return true;
    return !countriesWithSpecificLocations.has(String(location.country || '').toLowerCase());
  });
}
...
async function normalizeWhoOutbreakItem(item = {}, options = {}) {
  ...
  const locations = await geocodeOutbreakLocations(item, options);
...
async function normalizeWhoOutbreakResponse(data = {}, options = {}) {
  ...
  if (Array.isArray(data.value)) {
    for (const item of data.value) {
      reports.push(await normalizeWhoOutbreakItem(item, {
        ...options,
        geocodeBudget
      }));
    }
