...
// Within a single trend-analysis run, the same (data, districts) pair is
// point-in-polygon filtered up to 3x across the six aggregation functions.
// Cache results keyed by the array references (both stable for the run) so the
// scan runs once per unique pair. WeakMaps let the entries release when the
// underlying arrays are garbage-collected.
const filterToDistrictsCache = new WeakMap();

function computeFilterToDistricts(data, districts, latitudeKey, longitudeKey) {
...
export function filterToDistricts(data, districts, latitudeKey = 'latitude', longitudeKey = 'longitude') {
  ...
  let byDistricts = filterToDistrictsCache.get(data);
  if (!byDistricts) {
    byDistricts = new WeakMap();
    filterToDistrictsCache.set(data, byDistricts);
  }

  let byKeys = byDistricts.get(districts);
  if (!byKeys) {
    byKeys = new Map();
    byDistricts.set(districts, byKeys);
  }

  const cacheKey = `${latitudeKey}::${longitudeKey}`;
  if (byKeys.has(cacheKey)) {
    return byKeys.get(cacheKey);
  }

  const result = computeFilterToDistricts(data, districts, latitudeKey, longitudeKey);
  byKeys.set(cacheKey, result);
  return result;
}

/**
 * Assign each record to the districts that contain it, in a single pass.
 *
 * Instead of filtering the full dataset once per district (O(districts × records)),
 * this walks each record once and tests it against each district, bucketing matches
 * by district index. Combined with the bbox prefilter in geo/core, the per-record
 * work is cheap, so the whole comparison collapses toward O(records).
 *
 * @returns {Array<Array>} one bucket per district (same order as `districts`),
 *   each an array of the records that fall inside that district.
 */
export function assignRecordsToDistricts(data, districts, latitudeKey = 'latitude', longitudeKey = 'longitude') {
  const buckets = districts.map(() => []);
  if (!Array.isArray(data) || data.length === 0 || !Array.isArray(districts) || districts.length === 0) {
    return buckets;
  }

  // Resolve each district's geometry once up front so the inner loop can call
  // isPointInGeometry directly — avoiding a fresh [districts[i]] array
  // allocation on every record×district iteration (millions for large inputs).
  const geometries = districts.map((district) =>
    district?.geometry || (district?.type === 'Feature' ? district.geometry : null)
  );
  ...
  for (let d = 0; d < data.length; d += 1) {
    ...
    for (let i = 0; i < districts.length; i += 1) {
      ...
      // Reuses the same cached bbox + ray-cast path as isPointInDistricts.
      try {
        if (isPointInGeometry(point, geometry)) {
          buckets[i].push(item);
        }
      } catch (_) {
        // Ignore malformed geometry for this district.
      }
    }
  }
 ...
 */
export function calculateFacilityRiskTrends(facilities = [], impactedFacilities = [], districts = []) {
  ...
  const impactedFacilityRows = Array.isArray(impactedFacilities)
    ? impactedFacilities
        .map((item) => item?.facility)
        .filter(Boolean)
    : [];

  const impactedInScope = filterToDistricts(impactedFacilityRows, districts);
  const impactedKeys = new Set(
    impactedInScope.map((facility) => `${facility.name}__${facility.latitude}__${facility.longitude}`)
  );
  ...
  const labelCoverage = {
    ...
    unlabeled: filteredFacilities.filter(f => !f.riskLevel && !f.risk_level).length
  };
  const labeledFacilities = labelCoverage.high + labelCoverage.medium + labelCoverage.low;
  const impactedCount = filteredFacilities.filter(
    (facility) => impactedKeys.has(`${facility.name}__${facility.latitude}__${facility.longitude}`)
  ).length;
  const safeCount = Math.max(0, filteredFacilities.length - impactedCount);
...
export function calculateDistrictComparison(districts = [], facilities = [], acledData = [], disasters = [], timeWindowDays = null) {
  ...
  // Single-pass assignment of each dataset to districts (O(records)) instead of
  // re-filtering the full dataset once per district (O(districts × records)).
  const facilitiesByDistrict = assignRecordsToDistricts(facilities, districts);
  const acledByDistrict = assignRecordsToDistricts(acledData, districts);
  const disastersByDistrict = assignRecordsToDistricts(disasters, districts);

  return districts.map((district, index) => {
    // O(1) lookup of this district's pre-bucketed records
    const districtFacilities = facilitiesByDistrict[index];
    let districtAcled = acledByDistrict[index];
    const districtDisasters = disastersByDistrict[index];
