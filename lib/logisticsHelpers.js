...
function calculateBridgeRisk(bridge, nearestDisaster) {
  ...
  const disasterType = (disaster.eventtype || disaster.type || '').toLowerCase();
 ...
 */
function calculateAccessScore(roadAnalysis, fuelAnalysis, airAnalysis) {
  ...
  // Weighted average: roads (50%), fuel (25%), air (25%)
  const overallScore = (roadScore * 0.5) + (fuelScore * 0.25) + (airScore * 0.25);
  ...
  return {
    ...
    description,
    breakdown: {
      roadAccess: { score: roadScore, weight: '50%' },
      fuelAccess: { score: fuelScore, weight: '25%' },
      airAccess: { score: airScore, weight: '25%' }
    },
    recommendations,
    detailedAnalysis: {
