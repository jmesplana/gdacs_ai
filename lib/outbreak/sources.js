...
export async function loadMines() {
  ...
  const visits=parseCSV(source.text);
  return { data:latestMines(visits),visits,origin:'public',source:IPIS_URL,url:IPIS_URL,sha256:source.sha256,fetchedAt:source.fetchedAt };
}
...
export async function loadRelocations() {
  ...
  return {origin:'public',routes:matrixRoutes(rows,Object.keys(rows[0]||{})),unit:product.unit,start:new Date(Date.UTC(year,m-2,1)).toISOString().slice(0,10),end:new Date(Date.UTC(year,m,0)).toISOString().slice(0,10),source:source.url,sha256:source.sha256,fetchedAt:source.fetchedAt,limitation:product.limitation};
}
