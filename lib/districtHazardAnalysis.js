...
function getAccessibilityReadiness(enabledLayerSet) {
  const requiredLayers = ['accessibility_context'];
  const missingLayers = requiredLayers.filter((layer) => !enabledLayerSet.has(layer));
...
function classifyAccessibility(travelTimeMinutes, hardToReachShare) {
  const minutes = toNumber(travelTimeMinutes) || 0;
  const hardToReach = toNumber(hardToReachShare) || 0;

  if (minutes >= 120 || hardToReach >= 0.45) return 'very hard to reach';
  if (minutes >= 90 || hardToReach >= 0.3) return 'hard to reach';
  if (minutes >= 60 || hardToReach >= 0.15) return 'moderately constrained';
  return 'relatively reachable';
}

function buildAccessibilityAssessment(accessibilityEvidence = null, enabledLayerSet = new Set()) {
  const readiness = getAccessibilityReadiness(enabledLayerSet);
  const hasEvidence =
    accessibilityEvidence?.accessibilityContextMean !== undefined &&
    accessibilityEvidence?.accessibilityContextMean !== null;

  if (!hasEvidence) {
    return {
      status: readiness.ready ? 'missing_evidence_data' : 'missing_required_layers',
      requiredLayers: readiness.requiredLayers,
      missingLayers: readiness.missingLayers,
      summary: null,
      message: readiness.ready
        ? 'Accessibility Context was selected, but district-level travel-time evidence could not be computed yet.'
        : `Enable ${readiness.missingLayers.map(formatLayerLabel).join(' and ')} to assess hard-to-reach conditions.`
    };
  }

  const travelTimeMinutesMean = Math.round(toNumber(accessibilityEvidence.travelTimeMinutesMean) || 0);
  const hardToReachShare = toNumber(accessibilityEvidence.hardToReachShare) || 0;
  const accessibilityContextMean = toNumber(accessibilityEvidence.accessibilityContextMean) || 0;
  ...
  return {
    ...
    missingLayers: [],
    summary: {
      accessibilityContextIndex: Math.round(accessibilityContextMean * 100),
      meanTravelTimeMinutes: travelTimeMinutesMean,
      hardToReachSharePct: Math.round(hardToReachShare * 100),
      classification: classifyAccessibility(travelTimeMinutesMean, hardToReachShare)
    },
    drivers: [
      {
        key: 'mean_travel_time',
        label: 'Mean modeled travel time to healthcare',
        value: travelTimeMinutesMean,
        unit: 'minutes',
        source: 'Oxford MAP accessibility'
      },
      {
        key: 'hard_to_reach_share',
        label: 'Share of district above 120-minute travel time',
        value: Math.round(hardToReachShare * 100),
        unit: '%',
        source: 'Oxford MAP accessibility'
      },
      {
        key: 'accessibility_context',
        label: 'Hard-to-reach context index',
        value: Math.round(accessibilityContextMean * 100),
        unit: '/ 100',
        source: 'Oxford MAP accessibility'
      }
    ],
    message: `District access context is ${classifyAccessibility(travelTimeMinutesMean, hardToReachShare)} with mean travel time ${travelTimeMinutesMean} minutes and ${Math.round(hardToReachShare * 100)}% of the area above 120 minutes to care.`
  };
}
...
function getConfidence({
  ...
  if (readyHazardCount === 0) {
    if (hasFloodEvidence || hasDroughtEvidence) {
...
export function buildDistrictHazardAnalysis({
  ...
  const analyzedDistricts = districts.map((district, index) => {
    ...
    const accessibilityAssessment = buildAccessibilityAssessment(districtEvidence.accessibility, enabledLayerSet);
    ...
    const readyHazardCount = Object.values(hazardAssessments).filter((assessment) => assessment?.status === 'ready').length;
    const hasFloodEvidence = districtEvidence.flood?.floodContextMean !== undefined && districtEvidence.flood?.floodContextMean !== null;
    const hasDroughtEvidence = districtEvidence.drought?.droughtContextMean !== undefined && districtEvidence.drought?.droughtContextMean !== null;
    ...
    return {
      ...
      confidence,
      confidenceReasoning: readyHazardCount === 0
        ? 'Confidence is low because no hazard has enough evidence to be scored yet, even though forecast weather and some contextual signals are present.'
        : evidenceBase === 'forecast plus active-context signals'
      ...
      rationale: [
        ...
        districtEvidence.accessibility?.travelTimeMinutesMean !== undefined ? `Mean travel time to healthcare ${Math.round(toNumber(districtEvidence.accessibility.travelTimeMinutesMean) || 0)} minutes with ${Math.round((toNumber(districtEvidence.accessibility.hardToReachShare) || 0) * 100)}% of area above 120 minutes` : null,
        districtEvidence.nighttimeLights?.avgRadMean !== undefined ? `Nighttime lights mean radiance ${Math.round((toNumber(districtEvidence.nighttimeLights.avgRadMean) || 0) * 10) / 10} with lit-area share ${Math.round((toNumber(districtEvidence.nighttimeLights.litAreaShare) || 0) * 100)}% from VIIRS monthly lights` : null,
        ...
        accessibilityAssessment.status !== 'ready' ? accessibilityAssessment.message : null
      ].filter(Boolean),
      ...
      sources: [
        ...
        districtEvidence.accessibility?.travelTimeMinutesMean !== undefined ? 'Oxford MAP accessibility district summary' : null,
        districtEvidence.nighttimeLights?.avgRadMean !== undefined ? 'VIIRS monthly nighttime lights district summary' : null,
  ...
  const hasAccessibilityEvidence = analyzedDistricts.some((district) => district.geeEvidence?.accessibility?.travelTimeMinutesMean !== undefined);
  ...
  return {
    ...
    summary: {
      ...
      methodology: [
        ...
        'Hard-to-reach context is summarized when Accessibility Context is enabled and district-level Oxford MAP travel-time evidence is available.',
      ...
      sources: [
        ...
        hasAccessibilityEvidence ? 'Oxford MAP accessibility district summaries when Accessibility Context is enabled' : null,
