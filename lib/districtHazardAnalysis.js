...
export function buildDistrictHazardAnalysis({
  ...
  const analyzedDistricts = districts.map((district, index) => {
    ...
    const hazardAssessments = {
      ...
      heat: {
        status: weatherSummary ? 'ready' : 'missing_weather',
        score: weatherSummary ? heatResult.score : null,
        level: weatherSummary ? getRiskLevel(heatResult.score) : 'not-ready',
        drivers: weatherSummary ? heatResult.drivers : [],
        requiredLayers: [],
        missingLayers: weatherSummary ? [] : ['forecast weather'],
        message: weatherSummary ? null : 'Weather forecast is unavailable for this district.'
      }
    };
    ...
    ].filter((hazard) => typeof hazard.score === 'number')
