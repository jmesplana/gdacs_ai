...
function getDistrictName(facility = {}) {
  return facility.district || facility.admin2 || facility.region || facility.admin1 || 'Unassigned';
}
...
function getRationale({ impactSummary, securitySummary, populationEstimate, operationConfig }) {
  ...
  if (impactSummary.impactCount > 0) {
    parts.push(`${impactSummary.impactCount} mapped impact signal${impactSummary.impactCount > 1 ? 's' : ''}`);
  }

  if (impactSummary.nearestDistance !== null) {
    parts.push(`nearest threat ${impactSummary.nearestDistance.toFixed(1)} km away`);
  }

  if (securitySummary.nearbyCount > 0) {
    parts.push(`${securitySummary.nearbyCount} ACLED incident${securitySummary.nearbyCount > 1 ? 's' : ''} within 50 km`);
  }
...
function getDistrictLabel(district = {}, fallback = 'Selected Area') {
  const props = district.properties || {};
  return district.name || props.ADM2_EN || props.NAME_2 || props.NAME || props.name || props.district || fallback;
}
...
function buildDistrictSoWhat({ populationEstimate, ageGroups, hospitals, clinics, disasterCount, acledCount, hasFacilityData, districtRiskLevel, districtRiskScore }) {
  const messages = [];
...
function buildDistrictKeyGaps({ hasFacilityData, hasWorldPop, hasHospitalCoverage, disasterCount, acledCount }) {
  const gaps = [];
...
function buildDistrictLeadershipNote({ disasterCount, acledCount, populationEstimate, districtRiskLevel }) {
  if (districtRiskLevel === 'very-high') {
...
function buildDistrictOnlyRows(selectedDistricts, scopedDisasters, scopedAcledData, scopedWorldPopData, operationConfig) {
  return selectedDistricts.map((district, index) => {
    ...
    const weightedScore = clamp(hazardScore + securityScore + populationScore, 0, 100);
    ...
    return {
      ...
      keyGaps: buildDistrictKeyGaps({
        hasFacilityData: false,
        hasWorldPop: Boolean(populationEstimate),
...
function buildDistrictRowsFromScope(selectedDistricts, scopedDisasters, scopedAcledData, scopedWorldPopData, osmData, operationConfig, facilityRows = []) {
  ...
  const rows = selectedDistricts.map((district) => {
    ...
    const districtFacilities = facilityRowsByDistrict.get(districtName) || [];
    const populationEstimate = getDistrictPopulationEstimate(district, scopedWorldPopData);
    ...
    if (districtFacilities.length > 0) {
      ...
      priorityScore = Math.round(clamp(averageScore + Math.min(urgentCount * 6, 18) + Math.min(districtRisk.securityScore / 4, 12), 0, 100));
      ...
      const baseRows = buildDistrictOnlyRows([district], scopedDisasters, scopedAcledData, scopedWorldPopData, operationConfig);
    ...
    return {
      ...
      keyGaps: buildDistrictKeyGaps({
        hasFacilityData: districtFacilities.length > 0,
        hasWorldPop: Boolean(populationEstimate || ageGroups),
        hasHospitalCoverage: hospitalStats.hasHospitalCoverage,
        disasterCount: districtRisk.disasterCount,
        acledCount: districtRisk.acledCount
      }),
...
export function buildPrioritizationBoard({
  ...
  const facilityRows = scopedFacilities.map((facility) => {
    ...
    return {
      ...
      facility,
      district: getDistrictName(impactRecord?.facility || facility),
