...
import { hasUsableGeometry, scoreDistrictRisk } from './districtRiskScoring';
...
function buildDistrictSoWhat({
  ...
  if (accessibilityAssessment?.status === 'ready' && accessibilityAssessment.summary) {
    messages.push(`Healthcare access is ${accessibilityAssessment.summary.classification}, with mean travel time ${accessibilityAssessment.summary.meanTravelTimeMinutes} minutes and ${accessibilityAssessment.summary.hardToReachSharePct}% of the area above 120 minutes to care.`);
  } else if (accessibilityAssessment?.message) {
    messages.push(accessibilityAssessment.message);
  }
  if (acledCount > 0) {
...
function buildDistrictKeyGaps({
  ...
  if (accessibilityAssessment?.status === 'missing_required_layers') {
    gaps.push('Hard-to-reach access context is not loaded');
  } else if (accessibilityAssessment?.status === 'missing_evidence_data') {
    gaps.push('Hard-to-reach access evidence could not be computed for this district');
  } else if (!hasAccessibilityContext) {
    gaps.push('Hard-to-reach access context is not loaded');
  }
...
function buildDistrictOnlyRows(selectedDistricts, scopedDisasters, scopedAcledData, scopedWorldPopData, operationConfig, districtHazardAnalysis = null) {
  return selectedDistricts.map((district, index) => {
    ...
    const accessibilityAssessment = hazardEntry?.accessibilityAssessment || null;
    ...
    return {
      ...
      keyGaps: buildDistrictKeyGaps({
        hasFacilityData: false,
        hasWorldPop: Boolean(populationEstimate || nighttimeLights || accessibilityAssessment?.status === 'ready'),
        hasHospitalCoverage: false,
        hasAccessibilityContext: accessibilityAssessment?.status === 'ready',
        accessibilityAssessment,
        disasterCount: districtRisk.disasterCount,
        acledCount: districtRisk.acledCount,
        hazardEntry
...
function buildDistrictRowsFromScope(selectedDistricts, scopedDisasters, scopedAcledData, scopedWorldPopData, osmData, operationConfig, facilityRows = [], districtHazardAnalysis = null) {
  ...
  const rows = selectedDistricts.map((district) => {
    ...
    return {
      ...
      keyGaps: buildDistrictKeyGaps({
        hasFacilityData: districtFacilities.length > 0,
        hasWorldPop: Boolean(populationEstimate || ageGroups || nighttimeLights || accessibilityAssessment?.status === 'ready'),
        hasHospitalCoverage: hospitalStats.hasHospitalCoverage,
        hasAccessibilityContext: accessibilityAssessment?.status === 'ready',
        accessibilityAssessment,
        disasterCount: districtRisk.disasterCount,
...
export function buildPrioritizationBoard({
  ...
  const hasAccessibilityContext = enabledEvidenceLayers.includes('accessibility_context');
  const availableSignals = [
    ...
    hasAccessibilityContext ? 'Hard-to-reach accessibility context' : null,
  ...
  if (!hasAccessibilityContext) missingSignals.push('Hard-to-reach accessibility context');
